...
public class newJapanese {
    public static void japaneseDisplay() {
        ...
        for (int i = 0; i < japaneseRecipes.size(); i++) {
            System.out.println((i + 1) + ". " + japaneseRecipes.get(i));
        }
        ...
        if (recipeNumber < 1 || recipeNumber > japaneseRecipes.size()) {
            System.out.println("Invalid recipe number. Please try again.");
            return;
        }

        // Get the recipe corresponding to the entered number
        String selectedRecipe = japaneseRecipes.get(recipeNumber - 1);
